...
class InvestorAgent(BaseAgent):
    """The Investor Agent responsible for providing brutal critiques."""
    
    def __init__(self, llm_service: LLMService):
        super().__init__(name="Investor", role_description="Venture Capitalist")
        self.llm_service = llm_service
        self.prompt_path = os.path.join(os.path.dirname(__file__), "..", "prompts", "investor.md")
    ...
    def execute(self, idea: StartupIdea, context: Optional[List[Message]] = None, task: Optional[Task] = None) -> Message:
        ...
        # The investor typically doesn't get a specific task, they just read the context
        if task:
